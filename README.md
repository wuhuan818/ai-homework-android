...
9. 真实图片描述上传前会自动准备识别副本，历史页支持收藏夹筛选、单条删除和清空确认。
...
5. 在图片描述场景中，可选择描述风格、选择图片、旋转 90°、添加文字水印，并分享处理后的图片。
...
4. 点击“选择图片”，选择一张设备图片。
5. 如果图片较大或格式需要转换，生成图片描述后确认创作页出现“图片已转换/压缩用于识别，可能对细节识别有一定影响。”提示。
6. 点击“旋转 90°”，确认预览和状态更新。
7. 输入水印文字，点击“添加水印”，确认处理后的预览和状态更新。
8. 点击“分享图片”，确认 Android 系统分享面板打开。
9. 未选择图片时点击图片处理操作，应看到中文提示。
