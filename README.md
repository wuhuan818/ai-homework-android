...
9. 真实图片描述上传前会自动压缩大图，历史页支持收藏夹筛选、单条删除和清空确认。
...
- 图片描述会在上传给模型前自动准备识别副本；大图会被压缩到更适合上传的尺寸和体积。
- 压缩只作用于发送给模型识别的副本，不覆盖用户选择的原图，也不替换旋转、水印或分享使用的图片文件。
...
4. 点击“选择图片”，选择一张设备图片。
5. 如果图片较大，生成图片描述后确认创作页出现“图片较大，已压缩用于识别，可能对细节识别有一定影响。”提示。
...
- 大图会在上传给模型前自动压缩识别副本，可能对细节识别有一定影响。
